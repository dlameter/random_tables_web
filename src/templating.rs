...
pub fn construct_liquid_complier() -> liquid::partials::EagerCompiler::<liquid::partials::InMemorySource> {
...
pub fn add_files_to_compiler(compiler: &mut liquid::partials::EagerCompiler::<liquid::partials::InMemorySource>) -> std::io::Result<()> {
...
pub fn create_layout_collection() -> std::io::Result<HashMap<String, HashMap<String, String>>> {
...
pub fn add_file_to_layout_collection(layouts: &mut HashMap<String, HashMap<String,String>>, file_path: &std::path::Path) -> Result<(), String> {
...
pub fn get_file_metadata(file_string: &String) -> HashMap<String, String> {
    if file_string.starts_with("---\n") {
        ...
        if split.len() != 3 {
            ...
            let mut metadata = process_file_frontmatter(split[1].to_string().clone());
...
pub fn process_file_frontmatter(metadata_string: String) -> HashMap<String, String> {
    let mut metadata = HashMap::new();
...
pub fn get_file_contents(path: &std::path::Path) -> Result<String, String> {
    let path_str = match path.to_str() {
        Some(s) => s,
        None => "[error: could not get str representation of path]",
    };
...
pub fn render_file(liquid_parser: Arc<liquid::Parser>, template_path: &std::path::Path/*, globals, &Object*/) -> impl warp::Reply {
