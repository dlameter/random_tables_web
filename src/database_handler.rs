...
impl DatabaseHandler {
    pub fn new(database_config: &DatabaseConfig) -> Result<DatabaseHandler, PgError> {
        let connect_string = format!(
            ...
            database_config.host.as_str(),
            database_config.dbname.as_str(),
            database_config.user.as_str(),
    ...
    pub fn create_account(
    ...
    ) -> Result<account::Account, PgError> {
        ...
        let query_string = format!(
            ...
            account::COLUMN_ACCOUNT_PASSWORD
        );
        let row = self.connection.query_one(
            query_string.as_str(),
            &[&account.name, &account.password_hash],
        )?;
    ...
    pub fn update_account(
        ...
        let query_string = format!(
            "UPDATE {} SET {} = $1, {} = $2 WHERE {} = $3 RETURNING *",
            account::ACCOUNT_TABLE_NAME,
            account::COLUMN_ACCOUNT_NAME,
            account::COLUMN_ACCOUNT_PASSWORD,
            account::COLUMN_ACCOUNT_ID
        ...
        let row = self.connection.query_one(
            query_string.as_str(),
            &[&account.name, &account.password_hash, &account.id],
        )?;
        DatabaseHandler::row_to_account(&row)
    }
    ...
    pub fn create_table(
        &mut self,
        table: &random_table::Table,
    ) -> Result<random_table::Table, PgError> {
        let query_string = format!(
            "INSERT INTO {} ({}, {}) VALUES ($1, $2)",
            random_table::TABLE_TABLE_NAME,
            random_table::COLUMN_TABLE_CREATED_BY,
            random_table::COLUMN_TABLE_NAME
        );
        let row = self
            .connection
            .query_one(query_string.as_str(), &[&table.created_by, &table.name])?;
    ...
    pub fn find_table_elements_by_table_id(&mut self, table_id: &i32) -> Option<Vec<String>> {
        let query_string = format!(
            "SELECT * FROM {} WHERE {} = $1",
            random_table::TABLE_ELEMENT_TABLE_NAME,
            random_table::COLUMN_TABLE_ELEMENT_TABLE_ID
        );
    ...
    pub fn delete_table_and_elements(
        &mut self,
        table: &random_table::Table,
    ) -> Result<random_table::Table, PgError> {
        ...
        let elements_result =
            DatabaseHandler::delete_table_elements_transaction(&mut transaction, table);
    ...
    pub fn create_table_elements(&mut self, table: &random_table::Table) -> Result<(), String> {
        if let Some(ref elements) = table.elements {
            let query_string = format!(
                ...
                random_table::TABLE_ELEMENT_TABLE_NAME,
                random_table::COLUMN_TABLE_ELEMENT_TABLE_ID,
                random_table::COLUMN_TABLE_ELEMENT_INDEX,
                random_table::COLUMN_TABLE_ELEMENT_TEXT
            );
            let statement = self
                .connection
                .prepare(query_string.as_str())
                .map_err(|error| format!("Failed to create statment with error: {}", error))?;

            for (index, value) in elements.iter().enumerate() {
                self.connection
                    .execute(&statement, &[&table.id, &(index as i32), value])
                    .map_err(|error| {
                        format!(
                            "Failed to insert row in random_table_element with error: {}",
                            error
                        )
                    })?;
            }
    ...
    fn delete_table_elements_transaction(
        transaction: &mut postgres::Transaction,
        table: &random_table::Table,
    ) -> Result<Option<Vec<String>>, PgError> {
        let query_string = format!(
